...
import SwitchCodeBlock from "../../../components/SwitchCodeBlock.astro";
...
<SadCodeBlock code={`dolor edad = 20;
vacio promedio = 3.14;
...
```bash
git clone https://github.com/Aserturik/sadScript.git
cd sadScript
./build.sh
```
...
| Tipo de literal | Ejemplo | Token interno |
| --------------- | ------- | ------------- |
| Booleano true | `esperanza x = vivo` | `TRUE` |
| Booleano false | `esperanza x = muerto` | `FALSE` |
| Nulo | `nulo` | `NULO` |
| Cadena | `recuerdo x = "texto"` | `LITERAL_CADENA` |
| Caracter | `'a'` | `LITERAL_CARACTER` |
| Entero | `dolor x = 42` | `LITERAL_ENTERO` |
| Flotante | `vacio x = 3.14` | `LITERAL_DECIMAL` |
| Notación científica | `vacio x = 1.5e10` (también `42e10`) | `LITERAL_DECIMAL` |
...
| Operador | Token interno     | Uso típico                |
| -------- | ----------------- | ------------------------- |
| `->`     | `FLECHA`          | Punteros / acceso         |
| `=>`     | `FLECHA_DOBLE`    | Lambdas / mapeo           |
| `#`      | `NUMERAL`         | Directivas / metadatos    |
| `@`      | `ARROBA`          | Decoradores / anotaciones |
...
- **~195 líneas** de definiciones Flex
- Reconoce **~78 tokens**: palabras reservadas, operadores, literales, identificadores, delimitadores
